...
# -*- coding: utf-8 -*-
#
# Provee herramientas para el manejo de la camara
...
class _Camera:
    ...
    def moveX(self, direction=CAMERA_POSITIVE):
        """Mueve la posicion de la camara en el eje x"""
        pass

    def moveY(self, direction=CAMERA_POSITIVE):
        """Mueve la posicion de la camara en el eje y"""
        pass

    def moveZ(self, direction=CAMERA_POSITIVE):
...
class CameraR(_Camera):
    ...
    def __init__(self, r=1.0, phi=45, theta=45, centerPoint=point3(), upVector=vector3(0, 0, 1)):
        """Funcion constructora"""
        if isinstance(centerPoint, point3):
            if isinstance(upVector, vector3):
                if r > 0:
                ...
                raise Exception("upVector debe ser del tipo vector3")
        else:
            raise Exception("centerPoint debe ser del tipo point3")
    ...
    def __str__(self):
        ...
        r = CAMERA_ROUNDED
        return "Camera: {12}\nRadius: {0}\nPhi angle: {1}, Theta angle: {2}\nXYZ eye pos: ({3},{4},{5})\nXYZ center pos: ({6},{7},{8})\nXYZ up vector: ({9},{10},{11})".format(
            round(self.r, r), round(self.phi, r), round(self.theta, r), round(x, r), round(y, r), round(z, r),
            round(self.center.getX(), r), round(self.center.getY(), r), round(self.center.getZ(), r),
            round(self.up.getX(), r), round(self.up.getY(), r), round(self.up.getZ(), r), self.getName())
